...
export function CustomScheme({ scheme, onChange }: CustomSchemeProps): VNode {
  ...
  return (
    <div class="border-b border-rule pt-2.5 pb-0.5">
      <p class="mb-2 text-[11px] text-ink-3">
        Custom scheme — six colors per mode; borders, tints and muted text are
        derived from these.
      </p>
      {(["light", "dark"] as ThemeMode[]).map((mode: ThemeMode): VNode => (
        <div key={mode} class="flex items-center gap-2.5 py-1">
          <span class="w-10 text-[10px] font-semibold tracking-[0.08em] text-ink-3 uppercase">
            {mode}
          </span>
          {COLOR_KEYS.map((key: keyof SchemeColors): VNode => (
            <label
              key={key}
              class="flex flex-col items-center gap-0.75 text-[9.5px] text-ink-faint"
            >
              {LABELS[key] ?? key}
              <input
                type="color"
                class="h-6.5 w-10 cursor-pointer rounded-panel border border-rule-strong bg-panel p-px"
