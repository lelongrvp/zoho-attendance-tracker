...
globalThis.chrome = {
  ...
  cookies: {
    getAll: async () => [
      { name: "CSRF_TOKEN", value: "tok", domain: "people.zoho.com" },
    ],
  ...
  notifications: {
    ...
    clear: () => {},
    onButtonClicked: reg("notifButton"),
  },
  runtime: {
    onInstalled: reg("installed"),
    onStartup: reg("startup"),
...
assert.strictEqual(
  armed.length,
  2,
  "both gates armed, got " + JSON.stringify(armed),
);
for (const a of armed)
  assert.ok(a.when > Date.now(), `${a.name} must be scheduled in the future`);
console.log(
  "1 OK  both gates armed:",
  armed
    .map((a) => `${a.name}@+${Math.round((a.when - Date.now()) / 60000)}m`)
    .join(" "),
);
...
assert.ok(
  store.get("gateState").fired.includes("partTime"),
  "fired gate recorded in storage",
);
...
const stillArmed = calls.alarmsCreated.filter(
  (a) => a.name === "gate:fullTime",
);
...
globalThis.fetch = async () => ({
  ok: true,
  json: async () => ({ dayList: {}, entries: {} }),
});
res = await send();
assert.strictEqual(
  res.status,
  "error",
  "degenerate payload should be rejected",
);
assert.strictEqual(
  store.get("attendanceData"),
  good,
  "cache must be preserved",
);
assert.ok(
  store.get("lastError").at >= store.get("lastSuccessAt"),
  "lastError newer than lastSuccess",
);
...
assert.match(
  res.message,
  /session/i,
  "should name the session, got " + res.message,
);
