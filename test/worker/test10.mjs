...
const STORE = {
  attendanceData: { dayList: { 0: { orgdate: "2026-09-01", tsecs: 100 } } },
};
const fetched = [];
...
const monthPayload = (label) => ({
  dayList: {
    0: { orgdate: label + "-05", tsecs: 8 * 3600, status: "Present" },
  },
  entries: {
    [label + "-05"]: [
      { fdate: "05-Jul-2026 - 09:00", tdate: "05-Jul-2026 - 17:00" },
    ],
  },
});
...
globalThis.fetch = async (url, init) => {
  ...
  return {
    ok: true,
    json: async () => monthPayload(monthLabel(Number(preMonth))),
  };
};
...
const send = (request) =>
  new Promise((resolve) => {
    ...
    assert.strictEqual(
      returned,
      true,
      "async handlers must return true to keep the channel open",
    );
  });
...
assert.strictEqual(
  fetched.at(-1),
  "2",
  "preMonth must carry the requested distance",
);
const now = new Date();
...
assert.ok(
  STORE.archivedMonths[key],
  `archive must be keyed by calendar month, got ${Object.keys(STORE.archivedMonths)}`,
);
assert.strictEqual(
  STORE.archivedMonths[key].entries,
  undefined,
  "only dayList is archived",
);
...
assert.strictEqual(
  messageListener({ action: "somethingElse" }, {}, () => {}),
  false,
);
assert.strictEqual(
  fetched.length,
  before,
  "unknown actions must not reach the network",
);
...
assert.strictEqual(
  response.status,
  "error",
  "an unusable payload must not be cached",
);
const stale = new Date(now.getFullYear(), now.getMonth() - 3, 1);
assert.ok(
  !STORE.archivedMonths[
    `${stale.getFullYear()}-${String(stale.getMonth() + 1).padStart(2, "0")}`
  ],
);
...
assert.strictEqual(response.status, "success", response.message);
assert.deepStrictEqual(
  fetched.slice(-2).sort(),
  ["0", "1"],
  "a refresh still fetches this month and last",
);
...
assert.strictEqual(
  response.status,
  "error",
  "a different month must not be cached under the asked-for key",
);
assert.match(response.message, /may not reach back that far/);
const wrong = new Date(now.getFullYear(), now.getMonth() - 5, 1);
assert.ok(
  !STORE.archivedMonths[
    `${wrong.getFullYear()}-${String(wrong.getMonth() + 1).padStart(2, "0")}`
  ],
);
