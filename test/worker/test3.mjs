...
const store = new Map();
const calls = { alarmsCreated: [], notifications: [], badge: [], badgeColor: [], titles: [] };
const listeners = {};
const reg = (name) => ({ addListener: (fn) => { listeners[name] = fn; } });
globalThis.chrome = {
  storage: { local: {
    get: async (k) => Object.fromEntries((Array.isArray(k)?k:[k]).map((x) => [x, store.get(x)])),
    set: async (o) => { for (const [k, v] of Object.entries(o)) store.set(k, v); },
    remove: async (k) => { (Array.isArray(k)?k:[k]).forEach((x) => store.delete(x)); },
  }, onChanged: reg("storageChanged") },
  cookies: { getAll: async () => [{ name: "CSRF_TOKEN", value: "t", domain: "people.zoho.com" }], onChanged: reg("c") },
  alarms: { create: (name, info) => calls.alarmsCreated.push({ name, ...info }),
            clear: async () => {}, get: async () => undefined, onAlarm: reg("alarm") },
  ...
  notifications: { create: (id, o) => calls.notifications.push({ id, ...o }), clear: () => {}, onButtonClicked: reg("nb") },
  runtime: { onInstalled: reg("i"), onStartup: reg("s"), onMessage: reg("message"), getURL: (p) => p },
};
...
assert.deepStrictEqual(armed.map((a) => a.name), ["gate:fullTime"], "only the still-future gate arms");
assert.strictEqual(Math.round((armed[0].when - fakeNow) / 60000), 10, "full-time gate at 00:30");
...
assert.strictEqual(longDays[0].id, "longDay:2026-09-11", "dedup key is today's date");
assert.strictEqual(calls.badge.at(-1), "10m");
assert.strictEqual(calls.badgeColor.at(-1), "#cc241d", "badge red past the threshold");
assert.match(calls.titles.at(-1), /Full-time at/);
console.log("1 OK  midnight fallback: gate", armed[0].name, "@+10m, badge 10m red, long-day pinged once");

// ---- Phase 2: a second refresh must not repeat the long-day ping
await send();
assert.strictEqual(calls.notifications.filter((n) => n.id.startsWith("longDay:")).length, 1, "long-day deduped");
...
assert.strictEqual(calls.badge.at(-1), "", "badge clears once the past-midnight day is over");
assert.match(calls.titles.at(-1), /no check-in/);
console.log("3 OK  gate fired past midnight:", JSON.stringify(gateNotes[0].message));
...
res = await send();
assert.strictEqual(res.status, "success");
assert.strictEqual(calls.notifications.filter((n) => n.id.startsWith("longDay:")).length, 1, "no new long-day ping");
assert.strictEqual(calls.alarmsCreated.filter((a) => a.name.startsWith("gate:")).length, 2, "both gates armed");
assert.strictEqual(calls.badgeColor.at(-1), "#665c54", "badge stays neutral before the threshold");
...
calls.alarmsCreated.length = 0;
let fetched = 0;
globalThis.fetch = async () => { fetched++; return { ok: true, json: async () => payload }; };
store.set("policy", { earlyFullTimeHours: 10 });
await listeners.storageChanged({ policy: { newValue: { earlyFullTimeHours: 10 } } }, "local");
...
assert.strictEqual(Math.round((rearmed[0].when - fakeNow) / 3600000), 8, "full-time now 10h after 09:00 = 19:00");
