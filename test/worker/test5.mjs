...
globalThis.chrome = {
  ...
  notifications: { create: () => {}, clear: () => {}, onButtonClicked: reg("nb") },
...
assert.strictEqual(new FakeDate(fullGate.when).getHours(), 18, "worked mode: gate at 18:00, not offset 18:15");
assert.strictEqual(new FakeDate(fullGate.when).getMinutes(), 0);
assert.strictEqual(new FakeDate(partGate.when).getHours(), 16, "part gate at 16:00 (6h of work)");
assert.strictEqual(calls.badge.at(-1), "3h", "badge counts down worked remainder");
assert.match(calls.titles.at(-1), /06:00/i, "badge tooltip names the worked target");
console.log("OK  worked mode reaches gates and badge: full 18:00, part 16:00, badge 3h");
