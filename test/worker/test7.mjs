...
const listeners = {}; const reg = (n) => ({ addListener: (fn) => { listeners[n] = fn; } });
globalThis.chrome = {
  storage: { local: {
  ...
  alarms: { create: () => {}, clear: async () => {}, get: async () => undefined, onAlarm: reg("alarm") },
  action: { setBadgeText: async () => {}, setBadgeBackgroundColor: async (o) => calls.badgeColor.push(o.color),
  ...
  runtime: { onInstalled: reg("i"), onStartup: reg("s"), onMessage: reg("m"), getURL: (p) => p },
};
...
assert.strictEqual(calls.badgeColor.at(-1), "#6e737e", "nord neutral badge color");
...
console.log("OK  VI worker:", JSON.stringify(note.title), "|", JSON.stringify(note.message));
console.log("OK  badge title:", JSON.stringify(calls.titles.at(-1)), "color:", calls.badgeColor.at(-1));
